...
export default async function AdminProductsPage({
  ...
  const [{ data: products }, { data: categories }] = await Promise.all([
    supabase
      .from("products")
      .select("*, images:product_images(*), category:categories(id, name_fr, display_order)")
  ...
  const grouped = (categories ?? []).map((cat) => ({
    category: cat,
    products: (products ?? []).filter((p: ProductRow) => p.category_id === cat.id),
  })).filter((g) => g.products.length > 0);

  // Products without category
  const uncategorized = (products ?? []).filter((p: ProductRow) => !p.category_id);
    ...
    <div className="pt-6">
      ...
      {(!products || products.length === 0) ? (
        ...
        <div className="space-y-8">
          {[...grouped, ...(uncategorized.length > 0 ? [{ category: null, products: uncategorized }] : [])].map((group) => (
            <div key={group.category?.id ?? "uncategorized"}>
              <h2 className="font-cormorant text-xl text-noir mb-3 px-1">
                {group.category?.name_fr ?? "Sans catégorie"}
                <span className="font-dm text-sm text-grege ml-2">({group.products.length})</span>
              </h2>
                ...
                <table className="w-full font-dm text-sm">
                  <thead>
                    <tr className="border-b border-beige">
                      {["photo", "Produit", "Prix", "Promo", "Statut", "actions"].map((h) => (
